var zBase = {
	config:{
		index:0,
	slider_btn	auto:true,
		direct:'left'
	},
	init:function(){
		this.js_slider = this.$id('js_slider');
		this.slider_innerv = this.$c('slider_inner')[0],
		this.slider_btn = this.$tag('a',this.$c('slider_btn')[0]);
		this.img_arr = this.$tag('img',this.slider_inner);
	...
	play:function(){
		this.slide.timer = setInterval(function(){
			...
			zBase.animate(zBase.slider_inner,zBase.config.direct,-zBase.config.index*500);
			for(var j=0;j<zBase.slider_btn.length;j++){
				zBase.$remve(zBase.slider_btn[j],'slider_current');
			}
			zBase.$add(zBase.slider_btn[zBase.config.index],'slider_current');
	...
	hover:function(){
		for(var i=0;i<this.slider_btn.length;i++){
			this.slider_btn[i].index = i;//储存每个导航的索引值
			this.slider_btn[i].onmouseover = function(){
			...
			this.slider_btn[i].onmouseout = function(){
				zBase.play();
			}
		}
	},
